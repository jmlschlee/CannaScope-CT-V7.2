...
# ============================================================================
# Crash-proof OCR — each scanned COA's OCR runs in an isolated subprocess, so a
# native engine segfault (e.g. Apple Vision) kills only that child, never the run.
# A semaphore caps concurrent OCR (overload guard); a hung/overloaded OCR retries
# once with a longer timeout; a true crash returns '' (unreadable) and is retried
# in the deferred pass at the end. This is ON by default (disable with --no-ocr).
# ============================================================================
...
def _isolated_ocr_pdf(src, max_pages: int = 6) -> str:
    ...
    for timeout in (120, 300):          # retry once with a longer timeout if overloaded
        try:
            with _OCR_SEM:
                r = subprocess.run([sys.executable, _OCR_WORKER, src, str(max_pages)],
            ...
            for p in unread:
                ...
                if p.analytes or p.cannabinoids:               # now readable
                    ...
                    if (p.flags or p.thc_flags or v5.unquantified_findings(p)
                            or v5.pathogen_detections(p)) and p not in keep:
                        keep.append(p)
